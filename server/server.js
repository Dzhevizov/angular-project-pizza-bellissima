(function (global, factory) {
    typeof exports === 'object' && typeof module !== 'undefined' ? module.exports = factory(require('http'), require('fs'), require('crypto'), require('mongodb')) :
        typeof define === 'function' && define.amd ? define(['http', 'fs', 'crypto', 'mongodb'], factory) :
            (global = typeof globalThis !== 'undefined' ? globalThis : global || self, global.Server = factory(global.http, global.fs, global.crypto, global.mongodb));
}(this, (function (http, fs, crypto, mongodb) {
    'use strict';
    ...
    const MONGO_DB = process.env.MONGODB_DB || 'pizzabellissima';
    const mongoClient = new MongoClient(MONGO_URI);
    const mongoConnection = mongoClient.connect().then(() => mongoClient.db(MONGO_DB)).catch(err => {
        console.error('MongoDB connection failed:', err);
        process.exit(1);
    });

    async function createMongoInstance(seedData = {}) {
        const db = await mongoConnection;
        await ensureSeedData(db, seedData);

        async function collectionExists(collection) {
            return await db.listCollections({ name: collection }, { nameOnly: true }).hasNext();
        }
        ...
        return {
            ...
            add: async function (collection, data) {
                ...
                const col = db.collection(collection);
                let id = uuid$1();
                while (await col.findOne({ _id: id }) !== null) {
                    id = uuid$1();
                }
                ...
                return transformDoc(record);
            },
            set: async function (collection, id, data) {
                const col = db.collection(collection);
                ...
                record._id = id;
                await col.replaceOne({ _id: id }, record);
                return transformDoc(record);
            },
            merge: async function (collection, id, data) {
                const col = db.collection(collection);
                const existing = await col.findOne({ _id: id });
                if (existing === null) {
                    throw new ReferenceError('Entry does not exist: ' + id);
                ...
                const record = assignClean(deepCopy(existing), data);
                record._updatedOn = Date.now();
                record._id = id;
                await col.replaceOne({ _id: id }, record);
                return transformDoc(record);
            ...
            delete: async function (collection, id) {
                const col = db.collection(collection);
                const result = await col.findOneAndDelete({ _id: id });
                if (result.value === null) {
                    throw new ReferenceError('Entry does not exist: ' + id);
                }
                return { _deletedOn: Date.now() };
            },
            query: async function (collection, query) {
                if (!(await collectionExists(collection))) {
                    throw new ReferenceError('Collection does not exist: ' + collection);
                }
                const col = db.collection(collection);
                ...
                return docs.filter(entry => {
                    for (let prop in query) {
                        if (query.hasOwnProperty(prop)) {
                            ...
                            if (typeof targetValue === 'string' && typeof entryValue === 'string') {
                                if (targetValue.toLocaleLowerCase() !== entryValue.toLocaleLowerCase()) {
                                    return false;
                                }
                            } else if (targetValue != entryValue) {
                                return false;
                            }
                        }
                    }
    ...
    async function ensureSeedData(db, seedData) {
        for (let collectionName in seedData) {
            ...
            if (count === 0) {
                const docs = Object.entries(seedData[collectionName]).map(([id, doc]) => Object.assign({}, doc, { _id: id }));
                if (docs.length > 0) {
    ...
    function transformDoc(doc) {
        return Object.assign(deepCopy(doc), { _id: String(doc._id) });
    }
...
async function get(context, tokens, query, body) {
        ...
        try {
            if (query.where) {
                responseData = (await context.storage.get(context.params.collection)).filter(parseWhere(query.where));
            } else if (context.params.collection) {
                responseData = await context.storage.get(context.params.collection, tokens[0]);
            } else {
                // Get list of collections
                return await context.storage.get();
            }
    ...
    async function post(context, tokens, query, body) {
        ...
        try {
            responseData = await context.storage.add(context.params.collection, body);
        } catch (err) {
            throw new RequestError$1();
        }
    ...
    async function put(context, tokens, query, body) {
        ...
        try {
            existing = await context.storage.get(context.params.collection, tokens[0]);
        } catch (err) {
            throw new NotFoundError$1();
        }
        ...
        try {
            responseData = await context.storage.set(context.params.collection, tokens[0], body);
        } catch (err) {
            throw new RequestError$1();
        }
    ...
    async function patch(context, tokens, query, body) {
        console.log('Request body:\n', body);
        ...
        try {
            responseData = await context.storage.merge(context.params.collection, tokens[0], body);
        } catch (err) {
            throw new RequestError$1();
        }
    ...
    async function del(context, tokens, query, body) {
        ...
        try {
            responseData = await context.storage.delete(context.params.collection, tokens[0]);
        } catch (err) {
            throw new RequestError$1();
        }
    ...
    function initPlugin(settings) {
        ...
        return async function decoreateContext(context, request) {
            context.storage = await storage;
            context.protectedStorage = await protectedStorage;
        };
    }
    ...
    function initPlugin$1(settings) {
        ...
        return async function decorateContext(context, request) {
            context.auth = {
                register,
                login,
                logout
            ...
            if (userToken !== undefined) {
                let user;
                const session = await findSessionByToken(userToken);
                if (session !== undefined) {
                    const userData = await context.protectedStorage.get('users', session.userId);
            ...
            async function register(body) {
                if (body.hasOwnProperty(identity) === false ||
                ...
                } else if ((await context.protectedStorage.query('users', { [identity]: body[identity] })).length !== 0) {
                    ...
                    const result = await context.protectedStorage.add('users', newUser);
                    delete result.hashedPassword;

                    const session = await saveSession(result._id);
            ...
            async function login(body) {
                const targetUser = await context.protectedStorage.query('users', { [identity]: body[identity] });
            ...
            async function logout() {
                if (context.user !== undefined) {
                    const session = await findSessionByUserId(context.user._id);
                    if (session !== undefined) {
                        await context.protectedStorage.delete('sessions', session._id);
                    }
                } else {
            ...
            async function saveSession(userId) {
                let session = await context.protectedStorage.add('sessions', { userId });
                const accessToken = hash(session._id);
                session = await context.protectedStorage.set('sessions', session._id, Object.assign({ accessToken }, session));
                return session;
            }

            async function findSessionByToken(userToken) {
                return (await context.protectedStorage.query('sessions', { accessToken: userToken }))[0];
            }

            async function findSessionByUserId(userId) {
                return (await context.protectedStorage.query('sessions', { userId }))[0];
            }
        };
    }
